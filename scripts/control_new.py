...
class Control(Node):
    ...
    def rel_alt_cb(self, data):
        ...
        if self.drone_state == CTRL_STATE.TAKING_OFF and \
            ...
            self.drone_state = CTRL_STATE.GROUND
        if self.drone_state == CTRL_STATE.GROUND and self.rel_alt > TAKEOFF_THRESHOLD:
            self.drone_state = CTRL_STATE.WP
    ...
    def get_gps(self):
        lon = 0 if self.lon is None else self.lon
        lat = 0 if self.lat is None else self.lat
        return {
            "msg": [lon, lat, self.rel_alt],
            "status": "succecss"
        }
