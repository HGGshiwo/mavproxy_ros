...
class Pland(BaseManager):
    ...
    def create_detector(self):
        tag_type = rospy.get_param("~tag_type", "tagCustom48h12")
        return Detector(
            families=tag_type,
            nthreads=1,
            quad_decimate=1.0,
        ...
        with self.info_lock:
            if self.camera_fov_xy is None:
                rospy.logwarn_throttle(
                    10, "No camera info received!, fallback to param"
                )
                camera_fov_x = rospy.get_param("~camera_fov_x", 114)
                camera_fov_y = rospy.get_param("~camera_fov_y", 114)
                self.camera_fov_xy = [camera_fov_x, camera_fov_y]
            else:
